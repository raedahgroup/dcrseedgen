...
func NewTableTextCell(text string, bold bool) fyne.CanvasObject {
	if bold {
...
func (table *Table) AddRowWithTextCells(textCells ...fyne.CanvasObject) {
	tableCells := make([]TableCell, len(textCells))
	for i, textCell := range textCells {
		tableCells[i] = textCell
	}
	...
	for _, row := range table.Rows {
		for i, cell := range row.Cells {
			if len(columnCellSizes) > i {
				columnCellSizes[i] = cell.MinSize().Subtract(fyne.NewSize(0, 25))
				//.Union(columnCellSizes[i])
			} else {
